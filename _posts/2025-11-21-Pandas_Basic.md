...
2. DataFrame : 2차원의 테이블 형태의 자료 구조입니다.
   ...
   * 각 열은 서로 다른 데이터 유형을 가질 수 있습니다.`</br>`
...
결측치 Nan 때문에 데이터 타입은 정수가 아닌 float64로 실수입니다.`</br>`
...
다음 포스팅에는 데이터 선택(Selection), 결측치 처리(Missing data), 연산(Operatios)에 대해 알아보겠습니다.`</br>`
